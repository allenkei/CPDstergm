...
arma::mat theta_learning(int learning_iter, List H_pos_list, List H_neg_list, List y_pos_list, List y_neg_list,
                         arma::mat theta_mat, arma::mat z_mat, arma::mat u_mat, double alpha, int tau, int p1, int p2, double theta_tol){
  ...
  for(int iter = 0; iter < learning_iter; ++iter){
    ...
    resnorm = as_scalar(sqrt(arma::mean(pow(residual,2),0)));
    if(resnorm < theta_tol){Rcout << "    ---theta converged at iteration " << iter+1 << "\n"; break;}
...
arma::mat z_learning(int learning_iter, arma::mat theta_mat, arma::mat z_mat, arma::mat u_mat,
                     arma::mat X_mat, arma::vec d_vec, double alpha, int tau, int p1, int p2, double lambda){
  ...
  for(int iter = 0; iter < learning_iter; ++iter){
    ...
    if(counter == (tau-1)){Rcout << "    ---beta converged at iteration " << iter+1 << "\n"; break;}
