...
sim_SBM_list <- function(num_nets=1, n=50, rho=0){
...
#'
#' set.seed(1)
#' STERGM_list <- sim_STERGM_list(num_nets=1, n=50, network_stats=c("edges", "mutual"),
...
#' @examples
#' set.seed(1)
#' SBM_list <- sim_SBM_list(num_nets=5, n=50, rho=0)
...
CPD_STERGM <- function(y_data, directed, network_stats, node_attr=NA,
  ...
  for(lambda_index in 1:length(list_of_lambda)){
    ...
    if(output[[4]] == "Save"){
      ...
      temp[[lambda_index]][[2]] <- result # list (est_CP, BIC, theta_change, data-driven threshold)
