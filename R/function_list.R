...
save_H_y_list <- function(y_data, directed, network_stats, node_attr=NA){
  ...
  for(t in 2:num_time){
    ...
    if(directed){
      ...
      H_pos_output <- H_pos_output[complete.cases(H_pos_output), ] # 29*28/2 = 406 rows
...
Evaluation_ADMM <- function(H_pos_list, H_neg_list, y_pos_list, y_neg_list,
  ...
  threshold <- mean(t_change) + qnorm(0.90, lower.tail = T) * sd(t_change)
  theta_change <- t_change
  ...
  BIC = -2*log_lik + p*log( (tau+1)*choose(n,2) )*(num_CP+1)
...
#' SBM_list <- sim_SBM_list(num_nets=1, n=50, rho=0)
#' network_stats=c("edges", "mutual")
#' sim1_result1 <- CPD_STERGM_list(SBM_list, directed=TRUE, network_stats)
...
#' xtick <- result$est_CP; seq_date <- rownames(market[start:end,])
#' axis(side=1, at=xtick-2, labels = FALSE, lwd = 0, lwd.ticks = 1)
#' text(x=xtick-6,  par("usr")[3]-1.8, labels = seq_date[xtick], srt=45, cex=0.7, xpd=TRUE)
CPD_STERGM <- function(y_data, directed, network_stats, node_attr=NA,
                       list_of_lambda=10^c(-2:7), ADMM_iteration=200, theta_iter=20, z_iter=20,
                       theta_tol=1e-3, ADMM_tol=1e-7, verbose=FALSE){
  temp <- list()
